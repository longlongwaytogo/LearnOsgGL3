    ...
    _preservingNearFarFrameNumber(0)
{
	...
	_loadOptions = new osgDB::Options;
	std::string strDataPath = Comm::GlobalVars::getInstance().getDataPath()+ "GFX\\PostEffectProcessors\\";
	_loadOptions->setDatabasePath(strDataPath);
	_numRenderOrder = 0;	
}
...
{
	_loadOptions = new osgDB::Options(*copy._loadOptions);
	_numRenderOrder = 0;	 
}
...
osg::Camera* EffectCompositor::createNewPass( PassType type, const std::string& name,GraphicsPipelineStage* stage )
{
    ...
    camera->setRenderOrder( osg::Camera::PRE_RENDER, _numRenderOrder++ );
    ...
    if ( type==DEFERRED_PASS )
    {
        ...
        osg::Node* quad = createScreenQuad(1.0, 1.0);
        camera->addChild(quad);
		camera->setCullingMode(osg::CullSettings::NO_CULLING);
		camera->setClearMask(GL_DEPTH_BUFFER_BIT);
		//camera->getOrCreateStateSet()->setMode(GL_LIGHTING,osg::StateAttribute::OFF);
        camera->setImplicitBufferAttachmentRenderMask(osg::Camera::IMPLICIT_COLOR_BUFFER_ATTACHMENT);//no depth buffer
        ((osg::Geode*)camera->getChild(0))->getDrawable(0)->setName(name);
    
    }
	else if(type == COMPUTE_PASS)
	{
...
const PassList& EffectCompositor::getPassList() const
{
...
osg::NodeList EffectCompositor::getCameras( PassType type ) const
{
