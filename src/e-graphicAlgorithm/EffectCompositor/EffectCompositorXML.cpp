...
struct UniformAnimator : public osg::Uniform::Callback
{
    virtual void operator()( osg::Uniform* uniform, osg::NodeVisitor* nv )
    {
        ...
        if ( loop )
        {
            ...
            double fraction_part = modulated_time - floor(modulated_time); // 获取小数部分
...
void EffectCompositor::setAttribute(osg::StateSet* stateset, osgDB::XmlNode* xmlChild)
{
/*
<attribute>
<CullFace mode="FRONT" glModeValue="1"></CullFace>
<PolygonMode face="FRONT_AND_BACK" mode="LINE" glModeValue="3"></PolygonMode>
<LineWidth width="5" glModeValue="3"></LineWidth>
<BindImageTexture unit =" " access="WRITE_ONLY" buffer="" ></BindImageTexture>
</attribute>
*/
	...
	for ( unsigned int j=0; j<xmlChild->children.size(); ++j )
	{
		...
		osgDB::XmlNode* attribXmlChild = xmlChild->children[j].get();
		const std::string& atttribName = attribXmlChild->name;
		if ( !isXMLNodeType(attribXmlChild) ) continue;

		int override = atoi( attribXmlChild->properties["glModeValue"].c_str() );
		osg::StateAttribute::GLModeValue modeValue = osg::StateAttribute::ON;
		if ( override>0 ) modeValue |= osg::StateAttribute::OVERRIDE;

		osg::StateAttribute* pAttribute = nullptr;
		if(atttribName == "LINEWITH")
		{
			osg::LineWidth* lineWidth = new osg::LineWidth;
			int width = atoi(attribXmlChild->properties["width"].c_str());
			lineWidth->setWidth(width);
			pAttribute = lineWidth;
		}
		else if(atttribName == "compute_dispath")
		{
			 
			 const std::string& strAccess =  attribXmlChild->properties["access"];
			 osg::BindImageTexture::Access access = osg::BindImageTexture::READ_ONLY;
			 if(strAccess == "READ_WRITE")
				 access = osg::BindImageTexture::READ_WRITE;
			 else if(strAccess == "WRITE_ONLY")
				 access = osg::BindImageTexture::WRITE_ONLY;
			 int unit = atoi(attribXmlChild->properties["unit"].c_str());
			 
			 
			 osg::Texture* pBuffer = getTexture(attribXmlChild->properties["buffer"]);
			 if(!pBuffer)
			 {
				 continue;
			 }
			 
			osg::BindImageTexture* bindImageTex = new osg::BindImageTexture(unit,pBuffer,access,pBuffer->getInternalFormat());
			pAttribute = bindImageTex;
		}
		else
		{
		...
		stateset->setAttributeAndModes(pAttribute, modeValue);
	}
}


osg::Camera* EffectCompositor::createPassFromXML( osgDB::XmlNode* xmlNode,const osgDB::Options* options )
{
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
		...
		{
			setAttribute(stateset,xmlChild);
		}
...
osg::Texture* EffectCompositor::createTextureFromXML( osgDB::XmlNode* xmlNode, bool asGlobal )
{
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        if ( childName=="file" )
        {
            ...
            if ( options.empty() ) image = osgDB::readImageFile( xmlChild->getTrimmedContents(),_loadOptions );
        ...
        else if ( childName=="rawfile" )
        {
            ...
            std::string rawfile = osgDB::findDataFile( xmlChild->getTrimmedContents(),_loadOptions );
...
osg::Shader* EffectCompositor::createShaderFromXML( osgDB::XmlNode* xmlNode, bool asGlobal )
{
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        else if ( childName=="file" )
        {
            ...
            else
            {
                ...
                shader->loadShaderSourceFromFile( shaderFile,_loadOptions );
#else
				shader = osgDB::readRefShaderFile(shaderFile,_loadOptions);
				shader->setName(name);
#endif 
            }
        }
	...
	//std::string strDataPath = osgDB::getFilePath();
	osg::ref_ptr<osgDB::Options> tmpOptions = new osgDB::Options(*_loadOptions);
	tmpOptions->setDatabasePath(filePath);
    while ( (pos = code.find("#include", pos))!=std::string::npos )
    {
        ...
        filename = osgDB::findDataFile( filename,tmpOptions );
...
bool EffectCompositor::loadFromXML( osgDB::XmlNode* xmlNode, const osgDB::Options* options )
{
    if ( xmlNode->type==osgDB::XmlNode::ROOT )
    {
        for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
        {
            ...
            if ( xmlChild->name=="compositor" )
                return loadFromXML( xmlChild, options );
        }
    }
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        else
        {
            ...
            if ( !ref.empty() )
            {
                osgDB::XmlNode* tempNode = _templateMap[ref].get();
        ...
        if ( childName=="technique" )
        {
            ...
            for ( unsigned int j=0; j<xmlChild->children.size(); ++j )
            {
                ...
                if ( xmlPassChild->name.find("pass")!=std::string::npos )
                    createPassFromXML( xmlPassChild ,options);
                else
...
EffectCompositor* Effect::readEffectFile( const std::string& filename, const osgDB::Options* options )
{
    osg::ref_ptr<osgDB::XmlNode> xmlRoot = osgDB::readXmlFile( filename, options );
    if ( xmlRoot.valid() )
    {
		osg::ref_ptr<osgDB::Options> newOpt = new osgDB::Options(*options);
        osgDB::FilePathList& filePaths = newOpt->getDatabasePathList();
        filePaths.push_back( osgDB::getFilePath(filename));
        filePaths.push_back( osgDB::getFilePath(filename));
        osg::ref_ptr<EffectCompositor> compositor = new EffectCompositor;
        compositor->loadFromXML( xmlRoot.get(), newOpt );
        return compositor.release();
    }
    return NULL;
}

EffectCompositor* Effect::readEffectStream( std::istream& stream, const osgDB::Options* options )
{
    osg::ref_ptr<osgDB::XmlNode> xmlRoot = osgDB::readXmlStream( stream );
    if ( xmlRoot.valid() )
    {
        osg::ref_ptr<EffectCompositor> compositor = new EffectCompositor;
        compositor->loadFromXML( xmlRoot.get(), options );
        return compositor.release();
    }
    return NULL;
}
