...
{	
	...
	ivec2 FragPos = ivec2(gl_GlobalInvocationID.xy); // TEXEL COORD
	...
	vec2 FragNDCPos4Light = (FragPosInLightSpace.xy+1)*0.5; // [-1,1]==>[0,1]
	float RSMTexelSize = 1.0 /u_RSMSize;
	vec3 DirectIllumination = vec3(0.0); 
	if(FragPosInLightSpace.z < 0.0f || FragPosInLightSpace.x>1.0f || FragPosInLightSpace.y > 1.0f || FragPosInLightSpace.x < 0.0f 
	|| FragPosInLightSpace.y < 0.0f) // 在光锥范围之外的的点
	{
		DirectIllumination = vec3(0.1) * FragAlbedo;
	}
	else
	{
		DirectIllumination = FragAlbedo * max(dot(-u_LightDirInViewSpace, FragViewNormal),0.1); // 漫反射
	}
